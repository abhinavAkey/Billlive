...
public class CompleteBillTransactionController extends BaseController {
	...
	private JSendResponse<List<CompleteBillTransaction>> jsend(
			List<CompleteBillTransaction> completeBillTransactionList) {
	...
	@RequestMapping(value = "/company/getallcompleteBillTransactions", method = RequestMethod.GET, produces = {
			BillliveMediaType.APPLICATION_JSON })
	public @ResponseBody JSendResponse<List<CompleteBillTransaction>> getAllCompleteBillTransactions(
		...
		List<CompleteBillTransaction> completeBillTransactionList = completeBillTransactionService
				.getAllCompleteBillTransactions(companyId);
		return jsend(completeBillTransactionList);
	}

	@RequestMapping(value = "/company/getcompleteBillTransaction", method = RequestMethod.GET, produces = {
			BillliveMediaType.APPLICATION_JSON })
	public @ResponseBody JSendResponse<CompleteBillTransaction> getCompleteBillTransactionById(
			@RequestParam(Constants.BILL_NUMBER) String billNumber, HttpServletRequest request,
			HttpServletResponse response) throws CompleteBillTransactionException {
		...
		if (StringUtils.isNotBlank(billNumber)) {
			...
			// String companyId = sessionModel.getCompanyId();
			String companyId = (String) request.getParameter(Constants.COMPANY_ID);
			CompleteBillTransaction completeBillTransaction = completeBillTransactionService
					.getCompleteBillTransactionById(billNumber, companyId);
			return jsend(completeBillTransaction);
		} else {
			LOG.error(
					"Bill Number is null in getAllCompleteBillTransactions method of CompleteBillTransactionController");
			throw new BillliveServiceException("completeBillTransactionId passed cant be null or empty string");
		}
	}

	@RequestMapping(value = "/company/addcompleteBillTransaction", method = RequestMethod.POST, consumes = {
			BillliveMediaType.APPLICATION_JSON }, produces = { BillliveMediaType.APPLICATION_JSON })
	public @ResponseBody JSendResponse<String> addCompleteBillTransaction(
			@RequestBody CompleteBillTransaction completeBillTransaction, HttpServletRequest request,
			HttpServletResponse response) throws CompleteBillTransactionException {
		...
		if (completeBillTransaction != null) {
			...
			String isCompleteBillTransactionCreated = completeBillTransactionService
					.addCompleteBillTransaction(completeBillTransaction);
			return jsend(isCompleteBillTransactionCreated);
		} else {
			LOG.error(
					"completeBillTransaction is null in addCompleteBillTransaction method of CompleteBillTransactionController");
			throw new BillliveServiceException("CompleteBillTransaction data passed cant be null or empty");
		}
	}

	@RequestMapping(value = "/company/updatecompleteBillTransaction", method = RequestMethod.POST, consumes = {
			BillliveMediaType.APPLICATION_JSON }, produces = { BillliveMediaType.APPLICATION_JSON })
	public @ResponseBody JSendResponse<String> updateCompleteBillTransaction(
			@RequestBody CompleteBillTransaction completeBillTransaction, HttpServletRequest request,
			HttpServletResponse response) throws CompleteBillTransactionException {
		...
		if (completeBillTransaction != null) {
			...
			completeBillTransaction.setCompanyId(companyId);
			completeBillTransaction.setUid(uid);
			String isCompleteBillTransactionUpdated = completeBillTransactionService
					.updateCompleteBillTransaction(completeBillTransaction);
			return jsend(isCompleteBillTransactionUpdated);
		} else {
			LOG.error(
					"completeBillTransaction is null in updateCompleteBillTransaction method of CompleteBillTransactionController");
			throw new BillliveServiceException("CompleteBillTransaction data passed cant be null or empty");
		}
	}
